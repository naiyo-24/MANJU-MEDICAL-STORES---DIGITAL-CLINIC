...
class CrmOrdersScreen extends ConsumerStatefulWidget {
  const CrmOrdersScreen({super.key});

  @override
  ConsumerState<CrmOrdersScreen> createState() => _CrmOrdersScreenState();
}

class _CrmOrdersScreenState extends ConsumerState<CrmOrdersScreen> {
  ...
  Future<void> _loadData() async {
    ...
    await ref.read(crmOrdersProvider.notifier).loadOrders();
    setState(() {
      _isLoading = false;
    });
  }
  ...
  Widget build(BuildContext context) {
    final ordersAsync = ref.watch(crmOrdersProvider);
    _orders = ordersAsync.value ?? [];
            ...
            children: [
              ...
              Column(
                ...
                children: const [
                  ...
                  SizedBox(height: 4),
                  Text(
                    'Manage medicine orders from store, website or patient app',
                        ...
                        ...currentOrder.items.asMap().entries.map((entry) {
                          ...
                          return Container(
                            ...
                            decoration: BoxDecoration(
                              border: Border(
                                bottom: BorderSide(
                                  color: i == currentOrder.items.length - 1
                                      ? Colors.transparent
  ...
  Widget _buildActionIcon(IconData icon, Color color, {bool isSolid = false}) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border.all(
          color: isSolid ? const Color(0xFFE9D5FF) : const Color(0xFFE2E8F0),
        ),
        borderRadius: BorderRadius.circular(4),
      ),
