...
class _LabTestsScreenState extends ConsumerState<LabTestsScreen> {
  ...
  void _showAddTestDialog({LabTest? existingTest}) {
    ...
    String selectedCategory =
        ...
        (_allCategories.isNotEmpty ? _allCategories.first : 'Uncategorized');
    final priceCtrl = TextEditingController(
      text: existingTest?.price.toString(),
    );
    final sampleCtrl = TextEditingController(
      text: existingTest?.sampleType ?? 'Blood',
    );
    final timeCtrl = TextEditingController(
      text: existingTest?.reportingTime ?? '24 hours',
    );
    String selectedTemplate =
        existingTest?.templateId ??
        (_templates.isNotEmpty ? _templates.first.id : '');
                      ...
                      Expanded(
                        child: _allCategories.isNotEmpty
                            ? DropdownButtonFormField<String>(
                                initialValue:
                                    _allCategories.contains(selectedCategory)
                                    ? selectedCategory
  ...
  void _exportCSV() async {
    String csv =
        'ID,Code,Name,Category,Price,Sample Type,Reporting Time,Status\n';
    for (var test in _tests) {
      csv +=
          '${test.id},${test.testCode},${test.name},${test.category},${test.price},${test.sampleType},${test.reportingTime},${test.isActive ? 'Active' : 'Inactive'}\n';
    }
    final bytes = utf8.encode(csv);
    await FileSaver.instance.saveFile(
      name: 'lab_tests_export.csv',
      bytes: bytes,
    );
    if (mounted) {
  ...
  void _exportExcel() async {
    ...
    sheet.appendRow([
      ...
      TextCellValue('Price'),
      TextCellValue('Sample Type'),
      TextCellValue('Reporting Time'),
      TextCellValue('Status'),
    ]);

    for (var test in _tests) {
      sheet.appendRow([
        TextCellValue(test.id),
        TextCellValue(test.testCode),
        TextCellValue(test.name),
        TextCellValue(test.category),
        TextCellValue(test.price.toString()),
        TextCellValue(test.sampleType),
        TextCellValue(test.reportingTime),
  ...
  void _showCategoriesDialog() {
    showDialog(
      context: context,
      builder: (context) =>
          _CategoriesDialog(tests: _tests, onUpdate: _loadData),
    );
  }
  ...
  Widget build(BuildContext context) {
    ...
    if (testsAsync.isLoading ||
        ...
        customCategoriesAsync.isLoading) {
      return const Center(child: CircularProgressIndicator());
    }
    final filteredTests = _tests.where((t) {
      final matchesSearch =
          t.name.toLowerCase().contains(_searchQuery.toLowerCase()) ||
          t.testCode.toLowerCase().contains(_searchQuery.toLowerCase());
      final matchesCategory =
          _selectedCategory == null || t.category == _selectedCategory;
      final matchesStatus =
          _selectedStatus == null ||
          (_selectedStatus == 'Active' && t.isActive) ||
          (_selectedStatus == 'Inactive' && !t.isActive);

      String? templateIdFilter;
      if (_selectedTemplate != null) {
        templateIdFilter = _templates
            .where((temp) => temp.name == _selectedTemplate)
            .firstOrNull
            ?.id;
      }
      final matchesTemplate =
          _selectedTemplate == null || t.templateId == templateIdFilter;
    ...
    final totalPages =
        (filteredTests.length + _itemsPerPage - 1) ~/ _itemsPerPage;
    if (_currentPage > totalPages && totalPages > 0) _currentPage = totalPages;
    final startIndex = filteredTests.isEmpty
        ? 0
    ...
    final paginatedTests = filteredTests.isEmpty
        ? <LabTest>[]
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: const [
                                ...
                                SizedBox(height: 4),
                                Text(
                                  'Manage laboratory tests, pricing, templates and configurations',
                                  style: TextStyle(
                                    fontSize: 14,
                      ...
                      children: [
                        ...
                        SizedBox(
                          ...
                          child: _buildStatCard(
                            Icons.category,
                            'Categories',
                            '${_tests.map((t) => t.category).toSet().length}',
                            '',
                            Colors.green,
                          ),
                        ),
                                    ...
                                    _buildActionButton(
                                      ...
                                      () async {
                                        ...
                                        if (result != null) {
                                          ...
                                          if (bytes != null) {
                                            final success = await LabDataService.uploadCSV(
                                              bytes,
                                              name,
                                            );
                                            if (!mounted) return;
                                            ...
                                            if (success) {
                                              messenger.showSnackBar(
                                                const SnackBar(
                                                  content: Text('Tests imported successfully!'),
                                                ),
                                              );
                                              _loadData();
                                            } else {
                                              messenger.showSnackBar(
                                                const SnackBar(
                                                  content: Text('Failed to import tests. Please check CSV format.'),
                                                ),
                                              );
                                    ...
                                    () async {
                                      ...
                                      if (result != null) {
                                        ...
                                        if (bytes != null) {
                                          ...
                                          if (success) {
                                            messenger.showSnackBar(
                                              const SnackBar(content: Text('Tests imported successfully!')),
                                            );
                                            _loadData();
                                          } else {
                                            messenger.showSnackBar(
                                              const SnackBar(content: Text('Failed to import tests. Please check CSV format.')),
                                            );
                                            setState(() => _isLoading = false);
                                          }
                                        }
                              ...
                              children: [
                                Container(
                                  ...
                                  child: TextField(
                                    ...
                                    decoration: const InputDecoration(
                                      hintText:
                                          'Search test name, code or keyword...',
                                      hintStyle: TextStyle(
                                        fontSize: 13,
                                ...
                                _buildDropdown(
                                  'All Templates',
                                  _templates.map((t) => t.name).toList(),
                                  _selectedTemplate,
                              ...
                              children: [
                                ...
                                Padding(
                                  ...
                                  child: Wrap(
                                    ...
                                    children: [
                                      Text(
                                        'Showing ${filteredTests.isEmpty ? 0 : startIndex + 1} to $endIndex of ${filteredTests.length} tests',
                                        style: const TextStyle(
                                          color: Color(0xFF64748B),
                                          fontSize: 13,
                                        ),
                    ...
                    : SingleChildScrollView(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            _buildDetailRow(
                              'Test Code',
                              _selectedTest!.testCode,
                            ),
                            _buildDetailRow(
                              'Category',
                              _selectedTest!.category,
                            ),
                            _buildDetailRow(
                              'Price (₹)',
                              _selectedTest!.price.toStringAsFixed(2),
                            ),
                            _buildDetailRow(
                              'Template',
                              _templates
                            ...
                            _buildDetailRow(
                              'Sample Type',
                              _selectedTest!.sampleType,
                            ),
                            _buildDetailRow(
                              'Reporting Time',
                              _selectedTest!.reportingTime,
                            ),
                            _buildDetailRow(
                              'Description',
                              _selectedTest!.description,
                            ),
                                  ...
                                  child: _buildQuickActionCard(
                                    ...
                                    onTap: () {
                                      ScaffoldMessenger.of(
                                        ...
                                        const SnackBar(
                                          content: Text(
                                            'Please navigate to the Templates tab to manage templates.',
                                          ),
                                        ),
                                      );
                                    },
                                  ...
                                  child: _buildQuickActionCard(
                                    ...
                                    onTap: () async {
                                      try {
                                        ...
                                        final pdfBytes =
                                            await PdfGeneratorService.generateReport(
                                              _selectedTest!,
                                              template,
                                              patientData,
                                            );
                            ...
                            if (_packages
                                .where(
                                  (p) => p.testIds.contains(_selectedTest!.id),
                                )
                                .isEmpty)
                              const Text(
                              ...
                              ..._packages
                                  .where(
                                    (p) =>
                                        p.testIds.contains(_selectedTest!.id),
                                  )
                                  .take(3)
                                  .map(
                                    (pkg) => _buildRelatedPackage(
                                      pkg.name,
                                      '${pkg.testIds.length} Tests',
                                      pkg.discountedPrice.toString(),
                                    ),
                                  ),
                          ],
                        ),
                      ...
                      children: [
                        SizedBox(
                          ...
                          child: _buildBottomSummaryCard(
                            Icons.description,
                            'Custom Templates',
                            'Create and customize report templates for each test',
                          ),
                        ),
                        const SizedBox(width: 16),
                        SizedBox(
                          ...
                          child: _buildBottomSummaryCard(
                            Icons.inventory_2,
                            'Test Packages',
                            'Group tests with special pricing',
                          ),
                        ),
                        const SizedBox(width: 16),
                        SizedBox(
                        ...
                        SizedBox(
                          ...
                          child: _buildBottomSummaryCard(
                            Icons.analytics,
                            'Report Generation',
                            'Generate professional reports with your templates',
                          ),
                        ),
                        const SizedBox(width: 16),
                        SizedBox(
                          ...
                          child: _buildBottomSummaryCard(
                            Icons.send,
                            'Send Reports',
                            'Send reports via WhatsApp, Email or SMS',
                          ),
                        ),
                      ],
                    ),
          ...
          ElevatedButton(
            onPressed: () async {
              if (ctrl.text.trim().isNotEmpty) {
                await ref
                    .read(labCustomCategoriesProvider.notifier)
    ...
    return AlertDialog(
      title: Row(
        ...
        children: [
          ...
          IconButton(
            icon: const Icon(Icons.add, color: Color(0xFFEA580C)),
