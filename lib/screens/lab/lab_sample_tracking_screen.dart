...
class LabSampleTrackingScreen extends StatefulWidget {
  ...
  @override
  State<LabSampleTrackingScreen> createState() => _LabSampleTrackingScreenState();
}
...
class _LabSampleTrackingScreenState extends State<LabSampleTrackingScreen> {
  ...
  final List<String> _tabs = ['All Samples', 'Collected', 'In Processing', 'Ready', 'Delivered', 'Cancelled'];
  ...
  Widget _buildTimelineStep(String title, String? dateStr, String? byStr, bool isCompleted, bool isLast) {
    return Row(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Column(
        ...
        Expanded(
          child: Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Text(title, style: TextStyle(fontWeight: FontWeight.bold, color: isCompleted ? const Color(0xFF1E293B) : const Color(0xFF94A3B8), fontSize: 13)),
              if (dateStr != null && dateStr.isNotEmpty)
                Text(dateStr, style: const TextStyle(color: Color(0xFF64748B), fontSize: 12)),
              if (byStr != null && byStr.isNotEmpty)
                Text('by $byStr', style: const TextStyle(color: Color(0xFF64748B), fontSize: 12)),
  ...
  Widget build(BuildContext context) {
    ...
    List<LabBooking> displayedBookings = _bookings.where((b) {
      if (_selectedTab != 'All Samples' && b.status != _selectedTab) return false;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
              ...
              Row(
                children: [
                  ...
                  Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: const [
                      Text('Sample Tracking', style: TextStyle(fontSize: 28, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
                      SizedBox(height: 4),
                      Text('Track samples from collection to report delivery', style: TextStyle(fontSize: 14, color: Color(0xFF64748B))),
                    ],
                  ),
                ],
              ),
          ...
          Row(
            children: [
              _buildStatCard(Icons.biotech, 'Samples Collected Today', '28', Colors.blue, '↗+12%'),
              const SizedBox(width: 16),
              _buildStatCard(Icons.schedule, 'In Processing', '14', const Color(0xFFEA580C), '→ 0%'),
              const SizedBox(width: 16),
              _buildStatCard(Icons.check_circle, 'Reports Ready', '20', Colors.green, '↗+25%'),
              const SizedBox(width: 16),
              _buildStatCard(Icons.local_shipping, 'Reports Delivered', '18', Colors.purple, '↗+18%'),
            ],
          ),
                      ...
                      child: Column(
                                ...
                                Container(
                                  ...
                                  child: TextField(
                                    ...
                                    decoration: const InputDecoration(
                                      hintText: 'Search by patient name, sample ID...',
                                ...
                                child: Container(
                                  ...
                                  child: Row(
                                    children: [
                                      ...
                                      Expanded(flex: 2, child: Text(booking.sampleId, style: const TextStyle(fontSize: 13, fontWeight: FontWeight.bold, color: Color(0xFF1E293B)))),
                                      Expanded(flex: 2, child: Text(DateFormat('dd MMM yyyy hh:mm a').format(booking.bookingDate), style: const TextStyle(fontSize: 13, color: Color(0xFF64748B)))),
                                      Expanded(flex: 2, child: Text(booking.patientName, style: const TextStyle(fontSize: 13, color: Color(0xFF1E293B)))),
                                      Expanded(flex: 2, child: Text(booking.testsDescription, style: const TextStyle(fontSize: 13, color: Color(0xFF2563EB)), maxLines: 1, overflow: TextOverflow.ellipsis)),
                                      Expanded(
                                        flex: 2,
                                        child: Row(
                                          children: [
                                            Container(
                                              ...
                                              child: Row(
                                                ...
                                                children: [
                                                  ...
                                                  Text(booking.status, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: statusColor)),
                                                ],
                                              ),
                                            ),
                                          ],
                                      ...
                                      Expanded(flex: 2, child: Text(booking.assignedTo, style: const TextStyle(fontSize: 13, color: Color(0xFF1E293B)))),
                                      Expanded(
                                        flex: 1,
                                        child: Row(
                                          ...
                                          children: [
                                            Container(
                                              ...
                                              child: const Icon(Icons.visibility, size: 14, color: Color(0xFF64748B)),
                                            ),
                                            const SizedBox(width: 4),
                                            Container(
                                              padding: const EdgeInsets.all(4),
                                              decoration: BoxDecoration(border: Border.all(color: const Color(0xFFE2E8F0)), borderRadius: BorderRadius.circular(4)),
                                              child: const Icon(Icons.more_vert, size: 14, color: Color(0xFF64748B)),
                                            ),
                                          ],
                                        ),
                                      ),
                        ...
                        Padding(
                          ...
                          child: Row(
                            mainAxisAlignment: MainAxisAlignment.spaceBetween,
                            children: [
                              Text('Showing 1 to ${displayedBookings.length} of ${_bookings.length} samples', style: const TextStyle(color: Color(0xFF64748B), fontSize: 13)),
                              Row(
                                children: [
                        ...
                        child: Column(
                          ...
                          children: [
                              ...
                              children: [
                                Row(
                                  children: [
                                    ...
                                    const Text('Sample Details', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
                                  ],
                                ),
                                OutlinedButton.icon(
                                  onPressed: () {},
                            ...
                            if (_selectedBooking != null) ...[
                              _buildDetailRow('Sample ID', _selectedBooking!.sampleId),
                              _buildDetailRow('Patient Name', _selectedBooking!.patientName),
                              Padding(
                                padding: const EdgeInsets.only(bottom: 12),
                                child: Row(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    const SizedBox(width: 100, child: Text('Phone', style: TextStyle(color: Color(0xFF64748B), fontSize: 13))),
                                    Expanded(child: Row(
                                      children: [
                                        Text(_selectedBooking!.patientPhone, style: const TextStyle(color: Color(0xFF1E293B), fontSize: 13, fontWeight: FontWeight.w500)),
                                        const Spacer(),
                                        const Icon(Icons.wechat, color: Colors.green, size: 16),
                                        const SizedBox(width: 8),
                                        const Icon(Icons.phone, color: Color(0xFFEA580C), size: 16),
                                      ],
                                    )),
                                  ],
                                ),
                              ),
                              _buildDetailRow('Age / Gender', '${_selectedBooking!.patientAge} / ${_selectedBooking!.patientGender}'),
                              _buildDetailRow('Collection Date', DateFormat('dd MMM yyyy hh:mm a').format(_selectedBooking!.bookingDate)),
                              _buildDetailRow('Tests', _selectedBooking!.testsDescription, color: const Color(0xFF2563EB)),
                              _buildDetailRow('Sample Type', _selectedBooking!.sampleType),
                              _buildDetailRow('Collected By', _selectedBooking!.collectedBy),
                              _buildDetailRow('Assigned To', _selectedBooking!.assignedTo),
                              Padding(
                                padding: const EdgeInsets.only(bottom: 12),
                                child: Row(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    ...
                                    Container(
                                      ...
                                      child: Row(
                                        mainAxisSize: MainAxisSize.min,
                                        children: [
                                          Container(width: 6, height: 6, decoration: BoxDecoration(color: _getStatusColor(_selectedBooking!.status), shape: BoxShape.circle)),
                                          const SizedBox(width: 6),
                                          Text(_selectedBooking!.status, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: _getStatusColor(_selectedBooking!.status))),
                                        ],
                                      ),
                                    ),
                                  ],
                              ...
                              _buildDetailRow('Expected Report', _selectedBooking!.expectedReportDate != null ? DateFormat('dd MMM yyyy hh:mm a').format(_selectedBooking!.expectedReportDate!) : '-'),
                              _buildDetailRow('Remarks', '-'),
                            ] else ...[
                              const Center(child: Padding(padding: EdgeInsets.all(32), child: Text('Select a sample to view details', style: TextStyle(color: Color(0xFF94A3B8))))),
                            ],
                          ],
                        ),
                      ),
                        ...
                        Expanded(
                          child: Container(
                            ...
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                const Text('Tracking Timeline', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
                                const SizedBox(height: 24),
                                Expanded(
                                  child: ListView(
                                    children: [
                                      _buildTimelineStep('Sample Registered', DateFormat('dd MMM yyyy hh:mm a').format(_selectedBooking!.bookingDate.subtract(const Duration(minutes: 5))), _selectedBooking!.collectedBy, true, false),
                                      _buildTimelineStep('Sample Collected', DateFormat('dd MMM yyyy hh:mm a').format(_selectedBooking!.bookingDate), _selectedBooking!.collectedBy, _selectedBooking!.status != 'Pending', false),
                                      _buildTimelineStep('In Processing', _selectedBooking!.status != 'Pending' && _selectedBooking!.status != 'Collected' ? DateFormat('dd MMM yyyy hh:mm a').format(_selectedBooking!.bookingDate.add(const Duration(hours: 2))) : '', _selectedBooking!.assignedTo, _selectedBooking!.status != 'Pending' && _selectedBooking!.status != 'Collected', false),
                                      _buildTimelineStep('Report Ready', _selectedBooking!.status == 'Ready' || _selectedBooking!.status == 'Delivered' ? DateFormat('dd MMM yyyy hh:mm a').format(_selectedBooking!.expectedReportDate ?? DateTime.now()) : '', _selectedBooking!.assignedTo, _selectedBooking!.status == 'Ready' || _selectedBooking!.status == 'Delivered', false),
                                      _buildTimelineStep('Report Delivered', _selectedBooking!.status == 'Delivered' ? 'Delivered successfully' : 'Pending', '', _selectedBooking!.status == 'Delivered', true),
                                    ],
                                  ),
                                ),
                              ],
          ...
          Row(
            children: [
              Container(
                ...
                child: Row(
                  children: [
                    Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: const Color(0xFFEA580C), borderRadius: BorderRadius.circular(8)), child: const Icon(Icons.flash_on, color: Colors.white)),
                    const SizedBox(width: 12),
                    const Text('Quick Actions', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF1E293B))),
                  ],
                ),
              ),
              const SizedBox(width: 16),
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(child: Text(value, style: TextStyle(color: color ?? const Color(0xFF1E293B), fontSize: 13, fontWeight: FontWeight.w500))),
        ],
      ),
    );
  }
